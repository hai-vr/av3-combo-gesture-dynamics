...
namespace Hai.ComboGestureDynamics.Scripts.Editor
{
    public class CgdCompiler
    {
        ...
        private void DynamicsToSystems()
        {
            var parts = new[] {_cgd.mainPart}.Concat(_cgd.secondaryParts).ToArray();
            var systems = parts.Select(ConvertPartToSystem).ToArray();
        }

        private CgdSys.System ConvertPartToSystem(CgdPart part)
        {
            var system = new CgdSys.System();
            var activations = FlattenRulesToActivations(_cgd.rootRule, part);

            return system;
        }

        private CgdSys.Activation[] FlattenRulesToActivations(CgdRootRule rootRule, CgdPart part)
        {
            ...
            foreach (Transform child in rootRule.transform)
            {
                ...
                var activations = FlattenRulesBelongingToPart(rule, part, currentTweening, new CgdSys.ICondition[0]);
        ...
        private CgdSys.Activation[] FlattenRulesBelongingToPart(CgdRule currentRule, CgdPart part, CgdSys.Tweening inheritedTweening, CgdSys.ICondition[] inheritedConditions)
        {
            var mutableActivations = new List<CgdSys.Activation>();
            ...
            foreach (Transform child in currentRule.transform)
            {
                ...
                mutableActivations.AddRange(FlattenRulesBelongingToPart(childRule, part, currentTweening, inheritedConditions));
            }
