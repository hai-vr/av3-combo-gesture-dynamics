...
namespace Hai.ComboGestureDynamics.Scripts.EmbeddedAac.Framework.Editor.Internal.V0
{
    public static class AacV0
    {
        ...
        internal class AacFlBase<TAacType> where TAacType : AnimatorAsCode
        {
            ...
            public AacFlClip CopyClip(AnimationClip clipToCopy)
            {
                var clip = AacV0.CopyClip(_component, Guid.NewGuid().ToString(), clipToCopy);
                return new AacFlClip(_component, clip);
            }
            ...
            private AacFlLayer CreateLayer(string layerName, VRCAvatarDescriptor.AnimLayerType animLayerType)
            {
                ...
                var machine = ag.CreateOrRemakeLayerAtSameIndex("CGD_" + layerName, 1f, null);

                return new AacFlLayer(animator, _component, machine, "CGD_" + layerName);
            }
        ...
        internal readonly struct AacFlLayer
        {
            ...
            internal AacFlState NewStateAt(string name, float xx, float yy)
            {
                return StateMachine.NewStateAt(name, xx, yy);
            }
        ...
        private static AnimationClip CopyClip(AnimatorAsCode component, string suffix, AnimationClip clipToCopy)
        {
            var clip = Object.Instantiate(clipToCopy);
            // clip.name = component.layerNameSuffix + " " + suffix;
            clip.name = "zAutogenerated__" + component._internal.assetKey + "__" + suffix + "_" + Random.Range(0, Int32.MaxValue); // FIXME animation name conflict
            clip.hideFlags = HideFlags.None;
            AssetDatabase.AddObjectToAsset(clip, AnimatorOf(component, VRCAvatarDescriptor.AnimLayerType.FX));
